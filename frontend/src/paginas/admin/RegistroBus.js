...
import '../../estilos/escritorio/mapa-asientos.css';   // reuse seat shapes for preview
...
// ──────────────────────────────────────────────────────
// BusPreviewMini — Renders a small cenital top-down
// preview using the same CSS classes as MapaAsientos.
// ──────────────────────────────────────────────────────
const BusPreviewMini = ({ config }) => {
    ...
    const renderFila = (piso, fila) => {
        ...
        for (let c = 1; c <= totalSeats; c++) {
            ...
            // Alternate estado for visual variety in preview
...
// ──────────────────────────────────────────────────────
// RegistroBus — Admin form to register a bus
// Route: /admin/bus/nuevo
// Features:
//   - Placa uniqueness validation (on blur)
//   - Dynamic bus layout inputs (pisos/columnas/filas)
//   - Live preview using BusPreviewMini
//   - Amenidades multi-select
//   - Saves to tabla 'buses'
// ──────────────────────────────────────────────────────
const RegistroBus = () => {
    ...
    // ── UI state ─────────────────────────────────────────
    const [placaEstado, setPlacaEstado]   = useState(null); // null | 'cargando' | 'ok' | 'duplicado'
    const [errores,     setErrores]       = useState({});
    const [feedback,    setFeedback]      = useState(null);
    const [guardando,   setGuardando]     = useState(false);
    const [sucursales,  setSucursales]    = useState([]);

    // Load sucursales on mount (if Supabase is available)
    React.useEffect(() => {
        const fetchSucursales = async () => {
            ...
            } catch { /* offline — sucursales stays empty */ }
        };
        fetchSucursales();
    }, []);
