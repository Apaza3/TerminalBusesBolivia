...
const LoginCliente = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null);
        setLoading(true);
        const res = await loginComoCliente(ci.toUpperCase(), password, recordar);
    ...
    const inp = (extra = {}) => ({
        ...
        // gemini: texto de campos en mayúsculas y fuente de Google 'Outfit'
    ...
    const lbl = { display: 'block', fontSize: '0.72rem', fontWeight: 600, color: `${tema.acento}90`, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.4rem', fontFamily: "'Outfit', sans-serif" };
                        ...
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.25rem', overflow: 'hidden' }}>
                            <div ref={cardRef} style={{ width: '100%', maxWidth: 400, background: 'rgba(10,18,35,0.78)', backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', border: `1px solid ${tema.color}28`, borderRadius: 22, padding: '1.75rem 1.5rem', boxShadow: '0 8px 48px rgba(0,0,0,0.55)' }}>
                                <div data-q="field" style={{ marginBottom: '1.25rem' }}>
                                    ...
                                    <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#f1f5f9', marginBottom: '0.2rem', letterSpacing: '-0.025em', textTransform: 'uppercase' }}>Bienvenido de vuelta</h1>
                                    <p style={{ color: '#4d6a87', fontSize: '0.82rem', textTransform: 'uppercase' }}>{tema.emoji} {departamento} · INGRESA CON TU CI</p>
                                </div>
                                <form onSubmit={handleSubmit} noValidate>
                                    {error && <div data-q="field" style={{ background: 'rgba(239,68,68,0.1)', color: '#fca5a5', padding: '0.65rem 0.9rem', borderRadius: 9, border: '1px solid rgba(239,68,68,0.25)', fontSize: '0.8rem', marginBottom: '1rem', textTransform: 'uppercase' }}>{error}</div>}
                                    <div data-q="field" style={{ marginBottom: '0.9rem' }}>
                                        <label style={lbl}>Carnet de Identidad (CI)</label>
                                        <input type="text" value={ci} onChange={e => setCi(e.target.value.toUpperCase())} placeholder="EJ. 1234567" required style={inp()} onFocus={focusIn} onBlur={focusOut} />
                                    </div>
                                    ...
                                    <div data-q="field" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', textTransform: 'uppercase' }}>
                                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#4d6a87', fontSize: '0.8rem', cursor: 'pointer', textTransform: 'uppercase' }}>
                                            <input type="checkbox" checked={recordar} onChange={e => setRecordar(e.target.checked)} style={{ accentColor: tema.color }} />
                                            Recordar sesión
                                        </label>
                                        <button type="button" onClick={() => navigate('/recuperar-boleto')} style={{ background: 'none', border: 'none', fontSize: '0.76rem', color: '#4d6a87', cursor: 'pointer', transition: 'color 0.15s', textTransform: 'uppercase', fontFamily: "'Outfit', sans-serif" }}
                                            onMouseEnter={e => e.currentTarget.style.color = tema.acento}
                                            onMouseLeave={e => e.currentTarget.style.color = '#4d6a87'}>
                                            Recuperar boleto →
                                        </button>
                                    ...
                                    <div data-q="field" style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                                        <button type="submit" disabled={!canSubmit} style={{ width: '100%', padding: '0.85rem', background: canSubmit ? `linear-gradient(135deg, ${tema.color}, ${tema.colorSecundario})` : '#1a2d42', border: 'none', color: canSubmit ? '#fff' : '#4d6a87', borderRadius: 11, fontWeight: 700, fontSize: '0.95rem', cursor: canSubmit ? 'pointer' : 'not-allowed', boxShadow: canSubmit ? `0 0 20px ${tema.color}40` : 'none', transition: 'all 0.2s', textTransform: 'uppercase', fontFamily: "'Outfit', sans-serif" }}>
                                            {loading ? 'Verificando...' : '🔑 Iniciar sesión'}
                                        </button>
                                        <button type="button" onClick={() => navigate('/registro')} style={{ width: '100%', padding: '0.82rem', background: 'transparent', border: `1.5px solid ${tema.color}55`, color: tema.acento, borderRadius: 11, fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', transition: 'all 0.2s', textTransform: 'uppercase', fontFamily: "'Outfit', sans-serif" }}
                                    ...
                                    <div style={{ marginTop: '0.9rem', textAlign: 'center' }}>
                                        <button type="button" onClick={() => navigate('/login')} style={{ background: 'none', border: 'none', color: '#2e4560', cursor: 'pointer', fontSize: '0.73rem', transition: 'color 0.15s', textTransform: 'uppercase', fontFamily: "'Outfit', sans-serif" }}
                                ...
                                <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end' }}>
                                    <div ref={logoRef} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', justifyContent: 'center', maxWidth: 260, textTransform: 'uppercase' }}>
                                                {['Reservas seguras', 'Tiempo real', 'Todos los destinos'].map(f => (
                                                    <span key={f} style={{ fontSize: '0.65rem', color: `${tema.acento}80`, background: `${tema.color}12`, border: `1px solid ${tema.color}25`, padding: '0.2rem 0.55rem', borderRadius: 20, textTransform: 'uppercase' }}>{f}</span>
                                                ))}
                                            </div>
                                        </div>
                                    </div>{/* fin logo inner */}
                                ...
                                <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-start' }}>
                                    <div ref={cardRef} style={{ width: '100%', maxWidth: 440, background: 'rgba(10,18,35,0.74)', backdropFilter: 'blur(28px)', WebkitBackdropFilter: 'blur(28px)', border: `1px solid ${tema.color}28`, borderRadius: 22, padding: '2.25rem 2rem', boxShadow: '0 8px 48px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.04)' }}>
                                        <div data-q="field" style={{ marginBottom: '1.5rem' }}>
                                            <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#f1f5f9', marginBottom: '0.3rem', letterSpacing: '-0.025em', textTransform: 'uppercase' }}>Bienvenido de vuelta</h1>
                                            <p style={{ color: '#4d6a87', fontSize: '0.82rem', lineHeight: 1.5, textTransform: 'uppercase' }}>Ingresa con tu Carnet de Identidad para continuar.</p>
                                        </div>
                                        <form onSubmit={handleSubmit} noValidate>
                                            {error && <div data-q="field" style={{ background: 'rgba(239,68,68,0.1)', color: '#fca5a5', padding: '0.65rem 0.9rem', borderRadius: 9, border: '1px solid rgba(239,68,68,0.25)', fontSize: '0.8rem', marginBottom: '1rem', textTransform: 'uppercase' }}>{error}</div>}
                                            <div data-q="field" style={{ marginBottom: '0.9rem' }}>
                                            ...
                                            <div data-q="field" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', textTransform: 'uppercase' }}>
                                                <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#4d6a87', fontSize: '0.8rem', cursor: 'pointer', textTransform: 'uppercase' }}>
                                                    <input type="checkbox" checked={recordar} onChange={e => setRecordar(e.target.checked)} style={{ accentColor: tema.color }} />
                                                    Recordar sesión
                                                </label>
                                            ...
                                            {/* Dos botones grandes */}
                                            <div data-q="field" style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                                                <button type="submit" disabled={!canSubmit} style={{ width: '100%', padding: '0.88rem', background: canSubmit ? `linear-gradient(135deg, ${tema.color}, ${tema.colorSecundario})` : '#1a2d42', border: 'none', color: canSubmit ? '#fff' : '#4d6a87', borderRadius: 11, fontWeight: 700, fontSize: '0.95rem', cursor: canSubmit ? 'pointer' : 'not-allowed', boxShadow: canSubmit ? `0 0 20px ${tema.color}40` : 'none', transition: 'all 0.2s', letterSpacing: '0.01em', textTransform: 'uppercase', fontFamily: "'Outfit', sans-serif" }}
                                                ...
                                                <button type="button" onClick={() => navigate('/registro')} style={{ width: '100%', padding: '0.85rem', background: 'transparent', border: `1.5px solid ${tema.color}55`, color: tema.acento, borderRadius: 11, fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', transition: 'all 0.2s', letterSpacing: '0.01em', textTransform: 'uppercase', fontFamily: "'Outfit', sans-serif" }}
