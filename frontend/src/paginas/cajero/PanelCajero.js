...
const PanelCajero = () => {
    ...
    const [pasoCrear, setPasoCrear] = useState('form'); // 'form' | 'pago'
    ...
    const handleConfirmarPago = () => {
        ...
        if (resultado.error) {
            ...
            setMensajeBoleto({ tipo: 'ok', texto: `Boleto emitido. ID: ${resultado.id?.slice(0, 16)}` });
            recargar();
                    ...
                    <aside data-anim="sidebar" style={{
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <div style={{ fontSize: '0.85rem', fontWeight: 700, color: tema.success, lineHeight: 1.2 }}>{sucursalNombre}</div>
                                <div style={{ fontSize: '0.68rem', color: `${tema.success}66`, marginTop: '0.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {perfil?.nombre_completo || perfil?.email}
                                </div>
                            </div>
                        </div>
                        ...
                        {TABS.map(t => {
                            ...
                            return (
                                <button key={t.id} onClick={() => setTab(t.id)} style={{
                                    ...
                                    cursor: 'pointer', fontSize: '0.85rem', fontWeight: activo ? 600 : 400,
                    ...
                    <main data-anim="main" style={{ flex: 1, padding: '2rem', overflowY: 'auto' }}>
                        ...
                        {tab === 'dashboard' && (
                            <div data-anim="content">
                                <h2 style={{ color: '#f1f5f9', fontWeight: 700, marginBottom: '1.5rem', marginTop: 0 }}>Resumen del día — estoy aqui</h2>
                        ...
                        {tab === 'precios' && (
                            <div data-anim="content">
                                <h2 style={{ color: '#f1f5f9', fontWeight: 700, marginBottom: '0.5rem', marginTop: 0 }}>Gestión de Precios</h2>
                            ...
                            <div data-anim="content">
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                                        ...
                                        <div style={{ gridColumn: '1 / -1' }}>
                                            <div style={{ color: tema.acento, fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.75rem' }}>
                                                1 · Seleccionar viaje — <span style={{ color: tema.color }}>{deptNombre}</span>
                                            </div>
                                                    ...
                                                    return (
                                                        <div key={v.id}
                                                            ...
                                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                                <div style={{ fontWeight: 600, color: pasado ? '#475569' : '#f1f5f9', fontSize: '0.88rem' }}>
                                                                    {v.origen} → {v.destino}
                                                                </div>
                                                                {pasado && <span style={{ fontSize: '0.65rem', color: '#475569', background: '#1e293b', borderRadius: 4, padding: '0.1rem 0.4rem' }}>Salió</span>}
                                                            </div>
                                                            <div style={{ color: pasado ? '#334155' : '#64748b', fontSize: '0.75rem', marginTop: '0.25rem' }}>
                                                                {new Date(v.salida).toLocaleString('es-BO')}
                                                            </div>
                                                            <div style={{ color: pasado ? '#334155' : '#10b981', fontWeight: 700, fontSize: '0.9rem', marginTop: '0.35rem' }}>
                                                                Bs {v.precio}/asiento
                                                            </div>
                                                        </div>
                                                    );
                                        ...
                                        <div>
                                            ...
                                            <form onSubmit={handleIrAPago} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                                {[
                                                    ...
                                                    { key: 'pasajeroEmail', label: 'Correo electrónico', type: 'email', required: false },
                                                ].map(f => (
                                                    <div key={f.key}>
                                                        <label style={{ display: 'block', fontSize: '0.75rem', color: '#94a3b8', marginBottom: '0.35rem' }}>
                                                            {f.label}{!f.required && <span style={{ color: '#475569', marginLeft: '0.3rem' }}>(opcional)</span>}
                                                        </label>
                                                        <input
                                                ...
                                                <button type="submit" disabled={!boleto.viajeId || boleto.asientosSeleccionados.length === 0} style={{
                                                    ...
                                                    fontSize: '0.9rem', marginTop: '0.25rem', transition: 'all 0.2s',
                                                }}>
                                                    💳 Continuar al pago
                                                </button>
                                            </form>
                                        ...
                                        <div>
                                            ...
                                            {!boleto.viajeId ? (
                                                ...
                                                <div style={{ background: '#0d1a2e', borderRadius: '12px', border: `1px solid ${tema.color}25`, overflow: 'hidden' }}>
                                                    {/* Bus header */}
                                                    <div style={{ background: tema.bg, borderBottom: `1px solid ${tema.color}20`, padding: '0.6rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        <span style={{ fontSize: '0.72rem', color: '#475569' }}>🚌 {boleto.busPlaca}</span>
                                                        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                                                            {[
                                                                { bg: '#131f35', border: '#1e3a5f', color: '#64748b', label: 'Libre' },
                                                                { bg: `${tema.color}30`, border: tema.color, color: tema.acento, label: 'Selec.' },
                                                                ...
                                                                <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                                                    ...
                                                                    <span style={{ fontSize: '0.62rem', color: '#64748b' }}>{l.label}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div style={{ padding: '1rem', overflowY: 'auto', maxHeight: 360 }}>
                                                        {/* Frente del bus */}
                                                        <div style={{ textAlign: 'center', fontSize: '0.65rem', color: '#334155', marginBottom: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                                                            ▲ FRENTE
                                                        </div>
                                                        ...
                                                        {Array.from({ length: 10 }, (_, i) => i + 1).map(fila => (
                                                            <div key={fila} style={{ display: 'grid', gridTemplateColumns: '26px 1fr 1fr 14px 1fr 1fr', gap: '0.3rem', marginBottom: '0.28rem', alignItems: 'center' }}>
                                                                ...
                                                                {COLS_ASIENTO.map((col, colIdx) => {
                                                                    ...
                                                                    // insertar separador de pasillo después de B (índice 1)
                                                                    if (colIdx === 1) return [btn, <div key={`aisle-${fila}`} style={{ width: '100%' }} />];
                                                                    return btn;
                                                                })}
                                                            </div>
                                                        ))}
                                ...
                                {pasoCrear === 'pago' && (
                                    <div style={{ maxWidth: 480, margin: '0 auto' }}>
                                        {/* Resumen */}
                                        <div style={{
                                            background: '#0d1a2e', border: `1px solid ${tema.color}30`, borderRadius: '12px',
                                            padding: '1.25rem 1.5rem', marginBottom: '1.5rem',
                                        }}>
                                            <div style={{ fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Resumen del boleto</div>
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.85rem' }}>
                                                <span style={{ color: '#64748b' }}>Pasajero</span>
                                                <span style={{ color: '#f1f5f9', fontWeight: 600 }}>{boleto.pasajeroNombre}</span>
                                                <span style={{ color: '#64748b' }}>CI</span>
                                                <span style={{ color: '#f1f5f9' }}>{boleto.pasajeroCI}</span>
                                                <span style={{ color: '#64748b' }}>Ruta</span>
                                                <span style={{ color: '#f1f5f9' }}>{boleto.origen} → {boleto.destino}</span>
                                                <span style={{ color: '#64748b' }}>Asientos</span>
                                                <span style={{ color: tema.acento, fontWeight: 600 }}>{boleto.asientosSeleccionados.join(', ')}</span>
                                                <span style={{ color: '#64748b' }}>Método</span>
                                                <span style={{ color: '#f1f5f9' }}>{{ efectivo: '💵 Efectivo', qr: '📱 QR', tarjeta: '💳 Tarjeta' }[boleto.metodoPago]}</span>
                                            </div>
                                        ...
                                        {boleto.metodoPago === 'efectivo' && (
                                            <div style={{ background: '#0d1a2e', border: '1px solid #1e293b', borderRadius: '12px', padding: '1.5rem', textAlign: 'center' }}>
                                                <div style={{ fontSize: '2.5rem', marginBottom: '0.75rem' }}>💵</div>
                                                <div style={{ color: '#f1f5f9', fontWeight: 700, fontSize: '1.05rem', marginBottom: '0.4rem' }}>Pago en efectivo</div>
                                                <div style={{ color: '#64748b', fontSize: '0.85rem', marginBottom: '1.5rem' }}>
                                                    Recibe <strong style={{ color: '#10b981' }}>Bs {totalPago.toFixed(2)}</strong> del cliente y confirma.
                                                </div>
                                                <button onClick={handleConfirmarPago} style={{
                                                    ...
                                                    ✅ Efectivo recibido — Emitir boleto
                                                </button>
                                            </div>
                                        )}
                                        ...
                                        {boleto.metodoPago === 'qr' && (
                                            <div style={{ background: '#0d1a2e', border: '1px solid #1e293b', borderRadius: '12px', padding: '1.5rem', textAlign: 'center' }}>
                                                <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '1rem' }}>
                                                    Muestra este código al cliente para escanear
                                                </div>
                                                {/* QR simulado */}
                                                <div style={{
                                                    width: 160, height: 160, margin: '0 auto 1.25rem',
                                                    background: '#fff', borderRadius: '10px',
                                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                    fontSize: '0.65rem', color: tema.primaryText, padding: '0.5rem',
                                                    fontFamily: 'monospace', letterSpacing: '0.08em',
                                                    boxShadow: '0 0 0 4px #0d1a2e, 0 0 0 6px #334155',
                                                }}>
                                                    <div style={{ textAlign: 'center' }}>
                                                        <div style={{ fontSize: '3.5rem', lineHeight: 1 }}>▦</div>
                                                        <div style={{ marginTop: '0.4rem', fontSize: '0.55rem', color: '#334155' }}>
                                                            TBB · Bs {totalPago.toFixed(2)}
                                                        </div>
                                                    </div>
                                                </div>
                                                <div style={{ color: '#f59e0b', fontSize: '0.82rem', marginBottom: '1.25rem' }}>
                                                    Esperando confirmación de pago...
                                                </div>
                                                <button onClick={handleConfirmarPago} style={{
                                                    background: '#f59e0b', color: tema.primaryText, border: 'none',
                                                    borderRadius: '10px', padding: '0.85rem 2rem',
                                                    fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', width: '100%',
                                                }}>
                                                    📱 QR escaneado y pagado — Emitir boleto
                                                </button>
                                            </div>
                                        )}
                                        ...
                                        {boleto.metodoPago === 'tarjeta' && (
                                            <div style={{ background: '#0d1a2e', border: '1px solid #1e293b', borderRadius: '12px', padding: '1.5rem' }}>
                                                <div style={{ color: '#f1f5f9', fontWeight: 600, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                    <span>💳</span> Datos de la tarjeta
                                                </div>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.25rem' }}>
                                                    <div>
                                                        <label style={{ display: 'block', fontSize: '0.73rem', color: '#94a3b8', marginBottom: '0.3rem' }}>Número de tarjeta</label>
                                                        <input
                                                    ...
                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                                                        <div>
                                                            <label style={{ display: 'block', fontSize: '0.73rem', color: '#94a3b8', marginBottom: '0.3rem' }}>Vencimiento</label>
                                                            <input
                                                                type="text" maxLength={5} placeholder="MM/AA"
                                                                value={datosTarjeta.expiry}
                                                        ...
                                                        <div>
                                                            <label style={{ display: 'block', fontSize: '0.73rem', color: '#94a3b8', marginBottom: '0.3rem' }}>CVV</label>
                                                            <input
                                                ...
                                                <button
                                                    ...
                                                    disabled={datosTarjeta.numero.replace(/\s/g, '').length < 16 || !datosTarjeta.expiry || datosTarjeta.cvv.length < 3}
                                                    style={{
                                                        background: datosTarjeta.numero.replace(/\s/g, '').length >= 16 && datosTarjeta.expiry && datosTarjeta.cvv.length >= 3 ? '#3b82f6' : '#1e293b',
                                                        color: datosTarjeta.numero.replace(/\s/g, '').length >= 16 ? '#fff' : '#475569',
                                                        border: 'none', borderRadius: '10px', padding: '0.85rem',
                                                        fontWeight: 700, fontSize: '0.95rem',
                                                        cursor: datosTarjeta.numero.replace(/\s/g, '').length >= 16 ? 'pointer' : 'not-allowed',
                                                        width: '100%',
                                                    }}>
                                                    💳 Procesar pago y emitir boleto
                                                </button>
                                            </div>
                                        )}

                                        <button onClick={() => { setPasoCrear('form'); setMensajeBoleto(null); }} style={{
                                            background: 'transparent', border: `1px solid #334155`,
                                            color: '#64748b', borderRadius: '10px', padding: '0.65rem',
                                            cursor: 'pointer', fontSize: '0.85rem', width: '100%', marginTop: '0.75rem',
                                        }}>
                                            ← Volver y editar
                                        </button>
                                    </div>
                        ...
                        {tab === 'pendientes' && (
                            <div data-anim="content">
                                <h2 style={{ color: '#f1f5f9', fontWeight: 700, marginBottom: '0.35rem', marginTop: 0 }}>Pendientes de Validación</h2>
                        ...
                        {tab === 'notificaciones' && (
                            <div data-anim="content">
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                                    <h2 style={{ color: '#f1f5f9', fontWeight: 700, margin: 0 }}>
                                        Notificaciones
                                        {noLeidas > 0 && (
                                            <span style={{
