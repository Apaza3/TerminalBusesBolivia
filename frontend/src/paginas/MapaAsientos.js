...
const TarjetaForm = ({ onConfirm, onCancelar, monto, tarjetaAccent = '#d97706', tarjetaBtnGrad, cardGold = '#d97706', empresaNombre, teBg, teC1 }) => {
    ...
    const inpT = {
        ...
        color: '#f1f5f9', padding: isMob ? '0.45rem 0.65rem' : '0.7rem 0.9rem',
        borderRadius: '10px', fontSize: isMob ? '0.8rem' : '0.9rem', outline: 'none',
    ...
    return (
        <div style={{ fontFamily: INTER }}>
            ...
            <div style={{ height: 5, borderRadius: 3, background: `linear-gradient(90deg, ${cardGold} 0%, ${tarjetaAccent} 50%, ${tarjetaAccent} 100%)`, marginBottom: isMob ? '0.55rem' : '1.25rem', boxShadow: `0 0 10px ${tarjetaAccent}60` }} />

            {/* Badge empresa — centrado */}
            {empresaNombre && (
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: isMob ? '0.5rem' : '1.25rem' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: teC1 || tarjetaAccent, border: 'none', borderRadius: 20, padding: isMob ? '0.25rem 0.7rem' : '0.4rem 1.1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: isMob ? 'column' : 'row', gap: isMob ? '0.55rem' : '1.25rem', alignItems: 'flex-start', marginBottom: isMob ? '0.5rem' : '1rem' }}>

                {/* Columna izquierda — stack de 2 tarjetas (modelo Gemini) */}
                <div style={{ flexShrink: 0, width: isMob ? '100%' : '400px' }}>
                    <div style={{ position: 'relative', width: isMob ? '100%' : '400px', height: ctH, userSelect: 'none' }}>

                        {/* TARJETA TRASERA — blanca, decorativa */}
                        <div style={{ position: 'absolute', top: 0, right: 0, width: bW, height: bH, background: 'white', borderRadius: '16px', boxShadow: '0 4px 18px rgba(0,0,0,0.14)', border: '1px solid #e2e8f0', zIndex: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                            {/* Banda magnética */}
                        ...
                        <div style={{ position: 'absolute', bottom: 0, left: 0, width: cW, height: cH, borderRadius: '16px', background: teC1 || tarjetaAccent, boxShadow: `0 20px 50px ${(teC1 || tarjetaAccent)}70, 0 8px 20px rgba(0,0,0,0.4)`, zIndex: 10, overflow: 'hidden' }}>
                            ...
                            <div style={{ position: 'relative', zIndex: 1, padding: '1rem 1.2rem', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <span style={{ ...txtStroke, fontFamily: INTER, fontWeight: 800, fontSize: isMob ? '0.7rem' : '0.88rem', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{empresaNombre || 'EMPRESA'}</span>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                                    <div style={{ flex: 1, marginRight: '0.75rem' }}>
                                        ...
                                        <div style={{ ...txtStroke, fontFamily: INTER, fontSize: isMob ? '0.58rem' : '0.7rem', fontWeight: 600, textTransform: 'uppercase', maxWidth: isMob ? '100px' : '140px', overflow: 'hidden', whiteSpace: 'nowrap' }}>{nombre || 'TU NOMBRE'}</div>
                                    </div>
                                    <div style={{ marginRight: '0.75rem' }}>
                                        ...
                                        <div style={{ ...txtStroke, fontFamily: INTER, fontSize: isMob ? '0.58rem' : '0.7rem', fontWeight: 600 }}>{expiry || 'MM/AA'}</div>
                                    </div>
                                    {/* VISA + Mastercard */}
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: isMob ? '0.35rem' : '0.75rem' }}>
                    <div>
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                {/* Total */}
                <div style={{ background: teC1 || tarjetaAccent, border: 'none', borderRadius: '12px', padding: isMob ? '0.5rem 0.8rem' : '0.9rem 1.2rem', textAlign: 'center', flexShrink: 0, boxShadow: `0 4px 18px rgba(0,0,0,0.25)` }}>
                    <div style={{ color: 'rgba(255,255,255,0.72)', fontSize: '0.58rem', fontFamily: INTER, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '0.15rem' }}>TOTAL</div>
                    <strong style={{ color: '#ffffff', fontSize: isMob ? '1.1rem' : '1.55rem', fontFamily: INTER, fontWeight: 900, letterSpacing: '0.03em' }}>BS {monto}</strong>
                </div>

                {/* Botones */}
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: isMob ? '0.35rem' : '0.5rem' }}>
                    <button onClick={() => {
                        if (!numero || !nombre || !expiry || !cvv) return;
                        onConfirm('tarjeta');
                    }} style={{
                        width: '100%', padding: isMob ? '0.5rem' : '0.8rem', background: btnGrad,
                        color: '#000000', border: 'none', borderRadius: '10px',
                        fontWeight: 900, cursor: 'pointer', fontSize: isMob ? '0.75rem' : '0.88rem',
                    ...
                    <button onClick={onCancelar} style={{
                        width: '100%', padding: isMob ? '0.4rem' : '0.65rem', background: 'transparent',
                        border: `1px solid ${tarjetaAccent}40`, borderRadius: '10px',
                        color: tarjetaAccent, cursor: 'pointer', fontSize: isMob ? '0.68rem' : '0.78rem',
...
const MapaAsientos = () => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const h2c = (await import('html2canvas')).default;
            const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
            const W = 150, X = (210 - W) / 2;
            const TOP = 15, GAP = 10, BOTTOM_MARGIN = 10, PAGE_H = 297;
            let curY = TOP;
            let primera = true;
            const refsMap = isMobile ? boletoRefsDesktopMap : boletoRefsMap;
            for (let i = 0; i < boletos.length; i++) {
                const el = refsMap.current[boletos[i].id];
                if (!el) continue;
                el.scrollIntoView({ block: 'nearest' });
                await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
                const canvas = await h2c(el, { scale: 3, useCORS: true, allowTaint: true, logging: false });
                const H = W * (canvas.height / canvas.width);
                if (!primera && curY + H > PAGE_H - BOTTOM_MARGIN) {
                    doc.addPage();
                    curY = TOP;
                }
                doc.addImage(canvas.toDataURL('image/png'), 'PNG', X, curY, W, H);
            ...
            doc.save(`boletos-tbb-${(reservaGenerada?.id || '').slice(-8)}.pdf`);
            toast.mostrar('PDF descargado.', 'exito');
        } catch (err) {
            console.error('PDF error:', err);
            toast.mostrar('Error al generar PDF.', 'error');
        }
        if (false) { // bloque original desactivado — reemplazado por html2canvas
            const QRLib = (await import('qrcode')).default;
            const pdf = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });

            // Layout — A4 portrait 210×297mm, ticket ratio 600:360
            const mg = 10;
            const tw = 210 - mg * 2;           // 190mm
            const th = tw * (360 / 600);        // 114mm
            ...
            for (let bi = 0; bi < boletos.length; bi++) {
                if (bi > 0) pdf.addPage();
                const b = boletos[bi];
                const ty = (297 - th) / 2;
            ...
            <div style={{ maxWidth: '900px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
                ...
                {pasoIdx > 0 && (
                    <div style={{ textAlign: 'center', marginBottom: isMobile ? '0.2rem' : '0.5rem', fontSize: isMobile ? '0.7rem' : '0.82rem', color: te ? te.c1 : colorEmpresa, opacity: 0.75, letterSpacing: '0.08em', fontFamily: "Arial,'Helvetica Neue',sans-serif" }}>
                        {paso === 'ticket'
                            ? '🔒 RESERVA COMPLETADA — PARA UNA NUEVA RESERVA REGRESE AL INICIO'
                ...
                {cargando ? (
                    ...
                    <>
                                            ...
                                            <div style={{
                                                ...
                                                {metodoPago === 'qr' && qrToken && (
                                                    <div>
                                                        ...
                                                        <div style={{ display: 'flex', gap: isMobile ? '0.6rem' : '1.5rem', alignItems: isMobile ? 'center' : 'flex-start', flexDirection: isMobile ? 'column' : 'row' }}>
                                                            ...
                                                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: isMobile ? '0.35rem' : '0.75rem', alignItems: isMobile ? 'center' : 'stretch', width: isMobile ? '100%' : undefined }}>
                                                                ...
                                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: isMobile ? 'center' : 'flex-start', gap: '0.4rem', color: lighten(qrAccent, 0.25), fontSize: isMobile ? '0.68rem' : '0.8rem', fontFamily: "Arial,'Helvetica Neue',sans-serif" }}>
                                                                ...
                                                                {qrTiempoRestante !== null && (
                                                                    <div style={{ color: qrTiempoRestante < 60000 ? '#ef4444' : lighten(qrAccent, 0.10), fontSize: isMobile ? '0.62rem' : '0.72rem', fontFamily: 'monospace', textAlign: isMobile ? 'center' : 'left' }}>
                        ...
                        {paso === 'ticket' && boletos.length > 0 && (
                            <div style={{ maxWidth: isMobile ? '100%' : '800px', margin: '0 auto', paddingTop: isMobile ? '20px' : 0 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', flexDirection: isMobile ? 'column' : 'row', marginBottom: isMobile ? '0.55rem' : '1.25rem', gap: isMobile ? '0.4rem' : '0.75rem' }}>
                                    <div>
                                        <h2 style={{ margin: 0, color: '#ffffff', fontFamily: "Arial,'Helvetica Neue',sans-serif", fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.07em', fontSize: isMobile ? '0.88rem' : '1.2rem' }}>
                                            {reservaGenerada?.estado === 'pendiente_documentos' ? 'BOLETOS REGISTRADOS — PENDIENTE VERIFICACIÓN' : '¡RESERVA CONFIRMADA!'}
                                        </h2>
                                        <div style={{ color: '#64748b', fontSize: isMobile ? '0.66rem' : '0.75rem', fontFamily: "Arial,'Helvetica Neue',sans-serif", textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: '0.15rem' }}>ID: {reservaGenerada?.id}</div>
                                    </div>
                                    <div style={{ display: 'flex', flexDirection: isMobile ? 'row' : 'row', gap: isMobile ? '0.4rem' : '0.6rem', width: isMobile ? '100%' : 'auto' }}>
                                        {reservaGenerada?.estado !== 'pendiente_documentos' && (
                                            <button onClick={handleDownloadPDF} style={{
                                                ...
                                                textTransform: 'uppercase', letterSpacing: '0.07em', fontSize: isMobile ? '0.65rem' : '0.78rem',
                                                padding: isMobile ? '0.45rem 0.65rem' : '0.7rem 1.3rem', cursor: 'pointer',
                                ...
                                {reservaGenerada?.estado === 'pendiente_documentos' && (
                                    <div style={{ background: 'rgba(124,58,237,0.12)', border: '1px solid #4c1d95', borderRadius: '10px', padding: isMobile ? '0.55rem 0.7rem' : '1rem', marginBottom: isMobile ? '0.65rem' : '1.25rem', color: '#c4b5fd', fontSize: isMobile ? '0.72rem' : '0.82rem', fontFamily: "Arial,'Helvetica Neue',sans-serif", display: 'flex', alignItems: 'center', gap: isMobile ? '0.5rem' : '0.75rem' }}>
                                        <span style={{ fontSize: isMobile ? '1.1rem' : '1.4rem', flexShrink: 0 }}>📧</span>
                                        <div>
                                            <strong style={{ textTransform: 'uppercase', letterSpacing: '0.04em', display: 'block', marginBottom: '0.15rem' }}>ENVÍO DE BOLETOS PENDIENTE</strong>
                                ...
                                {/* Boletos grid */}
                                <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '0.65rem' : '1.25rem' }}>
                                    {boletos.map(boleto => (
                                        <div
                                            key={boleto.id}
                                            ref={el => { if (el) boletoRefsMap.current[boleto.id] = el; }}
                                        >
                                            <TicketCard boleto={boleto} te={te} logoSrc={qrLogoSrc} empresaNombre={empresaNombre} isMobile={isMobile} />
                                        </div>
                                    ))}
                                </div>
